...
<?php
	...
	$sql = "SELECT * FROM homeopathy WHERE featured = 1";
...
<ul class="row">      
	...
	<b><font size="6">HOMEOPATHIC MEDICINES</font></b></h1>
	...
	<!-----Adding Product ist via PHP(All homeopathy will be fetched from my own database)----->
	<!--------Applying while loop so that every item which is saved in the database named 'homeopathy' will be fetched and it will be added in the product list.------>
<div class="col-md-12" id="block">	
	<?php
	while($homeopathy=mysqli_fetch_array($featured))
	{
		?>
		<div class="col-md-3 text-center" id="blocks">
		<tr>
		<img src="<?= $homeopathy['image']; ?>" alt="<?= $homeopathy['Name']; ?>" class="img-thumb" />
		</tr>
		<tr>
		<h4><?= $homeopathy['Name']; ?></h4
		</tr>
		<tr>
		<p class="price">PRICE: Rs. <?= $homeopathy['Price']; ?></p>
		</tr>
		<tr>
		<input type="button" name="details" id="<?php echo $homeopathy['id']; ?>" class="details_data" value="Details" />
		</tr>
		</div>
...
<?php
	include 'includes/detailsmodal_homeopathy.php';
?>
...
$(document).ready(function(){
	$('.details_data').click(function()
	{
		...
		$.ajax(
		{
			url:"includes/detailsmodal_homeopathy.php",
			method:"post",
