...
<?php
	...
	$sql = "SELECT * FROM ayurvedic WHERE featured = 1";
...
<ul class="row">      
	...
	<b><font size="6">AYURVEDIC MEDICINES</font></b></h1>
	...
	<!-----Adding Product ist via PHP(All ayurvedic will be fetched from my own database)----->
	<!--------Applying while loop so that every item which is saved in the database named 'ayurvedic' will be fetched and it will be added in the product list.------>
<div class="col-md-12" id="block">	
	<?php
	while($ayurvedic=mysqli_fetch_array($featured))
	{
		?>
		<div class="col-md-3 text-center" id="blocks">
		<tr>
		<img src="<?= $ayurvedic['image']; ?>" alt="<?= $ayurvedic['Name']; ?>" class="img-thumb" />
		</tr>
		<tr>
		<h4><?= $ayurvedic['Name']; ?></h4
		</tr>
		<tr>
		<p class="price">PRICE: Rs. <?= $ayurvedic['Price']; ?></p>
		</tr>
		<tr>
		<input type="button" name="details" id="<?php echo $ayurvedic['id']; ?>" class="details_data" value="Details" />
		</tr>
		</div>
...
<?php
	include 'includes/detailsmodal_ayurveda.php';
?>
...
$(document).ready(function(){
	$('.details_data').click(function()
	{
		...
		$.ajax(
		{
			url:"includes/detailsmodal_ayurveda.php",
			method:"post",
